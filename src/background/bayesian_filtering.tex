...
\begin{theorem}[Bayes' Theorem]
    ...
    \end{equation}
\end{theorem}
In Bayesian inference, the prior distribution contains the information of some quantity before observing the data, and the posterior distribution contains the updated information after the observation.
...
Given the inherent uncertainties linked with measurements and modelling, instead of focusing solely on point estimates of the state, it is substantially more fruitful to consider probability distributions.
...
A few conditional probability distributions are usually of interest for the probabilistic state-space models, outlined in the following definition.
\begin{definition}
...
Theorem~\ref{thm:updateformulas} provides a framework for efficiently computing probability distributions conditioned on observations.
The prediction step equation~\ref{eq:predictionstep} as the joint probability distribution of the current and next state conditioned on the latest observations, facilitates probabilistic state estimates.
...
The Kalman filter provides an elegant closed-form solution for the update equations of Theorem~\ref{thm:updateformulas} in the special case of a linear state-space model with a Gaussian additive noise process.
...
Under these assumptions, the state's probability densities can be given by only estimates of the mean and covariance.
...
\begin{proof}
    In Appendix~\ref{app:proofs}.
\end{proof}
While the theorem assumes zero means for noises, the Gaussian distributions can easily be shifted to accommodate non-zero means.
...
However, for nonlinear state-space models, one can use linear approximations to preserve the convenience of Gaussian densities, known as the extended Kalman filter.
...
Although the original Kalman filter offers a robust solution to optimal filtering problems, it fails to scale for large state and observation spaces efficiently.
...
The observation update step of the EnKF has multiple formulations typically categorised into stochastic and deterministic versions, both of which are introduced here following~\cite{enkfbook} with a focus on the deterministic EnKF.
...
Before deriving the deterministic EnKF equations, a lemma for matrix inversion is needed:
...
\end{equation*}
and from these with denoting $v_{k+1}= \left(y_{k+1} - H_{k+1}\left(\mean{x}_{k+1\,\vert\,k}\right) \right)$ and using Lemma~\ref{lemma:woodbury} on line 4,
\begin{subequations}
    \begin{align*}
...
Multiple algorithms exist to create an ensemble around the posterior mean, but the Ensemble transform Kalman filter (ETKF) variant is presented and used here.
A posterior ensemble is created by perturbing the posterior mean with an estimate of posterior uncertainty $P_{k+1\,\vert\,k+1}$.
...
As full high-dimensional covariance matrices are represented by just a small ensemble, the ensemble covariance might is not a complete representation the full covariance.
...
As another strategy, covariance localisation filters the ensemble covariance from spurious correlations.
If some distant observations are known to have little correlation, the ensemble covariance matrix values can be adjusted there.

Spurious correlations and small ensembles also easily result in underestimating ensemble variance.
By inflating the ensemble spread, the overly optimistic variances can be widened.
Inflation can be achieved by multiplying the individual ensemble member deviations from the mean by some scalar larger than 1.
