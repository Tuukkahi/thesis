This thesis employs neural networks to estimate motion fields from advective spatio-temporal data.
Given the nonlinear and dynamic nature of the underlying process, the task is inherently complex.
Nevertheless, neural networks provide a flexible framework for modelling these dynamics.
...
As the material of this subsection also covers the elementary underlying principles of neural networks, it is beyond the bare minimum needed for understanding the results of this work but still provides some context for later discussions.
However, in this work, neural networks are forced to learn a partial differential equation in a departure from the mainstream neural network applications.
Therefore, knowing their inner workings is crucial for understanding what can be done and what kind of structures could potentially be used.
...
More complex neural networks have become feasible for various applications with the dramatic increase in computing capabilities.
...
Over the decades of evolution since their inception in the 1940s as a mathematical model for nervous activity~\cite{mcculloch, hebb}, neural networks have undergone many transformations in their architectures and capabilities.
In the 1950s to 1970s, the first concepts were modelled with a single neuron.
However, the application of backpropagation enabled the optimising of multiple layers of neurons, laying the foundations for the modern research of neural networks.
...
The first mathematical model for neurons from~\cite{mcculloch}, the McCulloch-Pitts Neuron, is the fundamental building block for any neural network.
...
\end{equation*}
computes sums an $n$-dimensional binary input vector $x$.
...
A few years later, from the McCulloch-Pitts model, Donald Hebb~\cite{hebb} proposed that the proximity between inputs and other neurons should also influence the output.
...
The perceptron model extends the limited  McCulloch-Pitts Neuron by allowing any input values weighted in the aggregation unit to add a bias term to the output.
The perceptron aggregation function $f_{\text{perceptron}} : \R^n \rightarrow \R$ is then
\begin{equation*}
    f_{\text{perceptron}}(x) = \sum_{i=1}^{n} w_{i} x_{i} + b,
\end{equation*}
where $w \in \R^n$ is the weight vector and $b \in \R$ is the bias term.
...
The activation function $\phi$ can also be changed to anything that introduces nonlinearity to otherwise linear networks.
...
The choice of activation function comes with technicalities crucial for training a neural network, such as the range of possible values and behaviour of derivatives.
...
Therefore, a so-called multilayer perceptron (MLP) was developed in which several perceptrons are combined as layers and applied sequentially, as shown in Figure~\ref{fig:mlp}.
\begin{figure}
    \centering
...
\begin{equation*}
    f^{(l)}_{\text{MLP}}(x; W, \beta) = W x + b,
\end{equation*}
where rows of the weight matrix $W \in \R^{n \times m}$ contain the weights and the bias vector $b \in \R^{m}$ has the bias terms of the neurons in the layer.
The activation function is then applied element-wise for the outputs of $f^{(l)}_{\text{MLP}}$.
The entire neural network is then a composition of all the individua layers.

MLPs are related to the famous Universal Approximation Theorem presented at the turn of the 1990s.~\cite{cybenko,horniketal,hornik}
...
Furthermore, the theorem does not state that any suitable function can be learned, only that it can be represented.
...
Their convolutional structure enables them to identify the critical features and structures from data effectively.
Consequently, a CNN is also used in the neural network developed for this thesis.
...
Particularly in the case of image data, two-dimensional convolutions are used.
Therefore, the examples from here on concern only the two-dimensional case.
...
\end{equation*}
The cross-correlation does not flip the kernel, resulting in loss of the commutative property.
Nevertheless, this technicality does not affect the result, as the convolution kernel can be flipped to yield the same outcome.
Hence, these layers are called convolution layers despite being technically implemented as cross-correlations.

CNNs employ many small convolutions as a replacement for the dot products performed in the MLP neurons.
With image inputs, the convolution kernels often have sizes of $3 \times 3$ or $9 \times 9$.
...
The input is convoluted with a large number of these small kernels individually, thus increasing the depth of the input.
...
\begin{figure}
    ...
    For instance, if all elements of $g$ were ${}^1{\mskip -5mu/\mskip -3mu}_9$, the output would have the average value of all $3 \times 3$ windows of the input.}
\end{figure}
...
Therefore, a neural network has to undergo a training process to optimise parameters over a large training dataset.

These learning tasks are classified into three categories: supervised, unsupervised, and reinforcement learning.
In supervised learning, data is labelled, and the model learns by comparing its output to the correct label.
Conversely, in unsupervised learning, the model has to learn patterns in data without being given labels.
...
All these learning tasks with neural networks ultimately lead to an optimisation problem.
...
\end{equation}
Here, $f$ denotes the neural network function that generates predictions given some input data $x$, drawn from the empirical training data distribution $\hat{\pi}_{\text{data}}$, and the network parameters.
$L$ is the loss function which compares the network prediction to the true label $y$.
This is easily modified to unsupervised learning by simply excluding $y$ from the equation, in which case the loss function has to use another method to compare correctness.
...
Perhaps the most commonly used and well-known loss function is the mean squared error (MSE), which computes the mean of squared prediction errors between a target value $Y$ and predicted value $\hat{Y}$:
\begin{equation}
    L_{\operatorname{MSE}}(Y, \hat{Y}) = \frac{1}{n} \sum_{i=1}^{n} \left( Y_{i} - \hat{Y}_{i} \right)^{2}.
\end{equation}
The standard loss functions are sometimes augmented with application-specific values depending on the application.
For instance, in the realm of physics-informed neural networks, it is customary to add to the used loss functions some physical quantities to be minimised. 
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \State Sample a mini-batch from training dataset $\{ (x^{(i)}, y^{(i)}) \}_{i=1}^{m}$
...
The stochastic nature of the SGD algorithm also introduces some noise into the computed losses in an optimum.
Consequently, the algorithm would never stabilise at the optimum but keep altering parameters.
A usual mitigation for this is decreasing the step size during training according to some schedule.
As the technical details related to step size scheduling techniques are outside the scope of this work, the interested are referred to e.g.~\cite{goodfellow} for in-depth review.
...
Hence, the next challenge is to find an efficient method for estimating gradients.

\subsubsection{Backpropagation}
Training a neural network starts with computing the network's predictions using inputs from the training dataset.
This part is often called forward propagation.
However, in all optimisation algorithms, the gradient of a scalar loss function is needed at some point.
Conventional approaches for numerical gradient estimation are computationally infeasible for neural networks, consisting of many parameters.
Therefore, a technique called backpropagation is needed for efficient gradient estimation.

Reverse accumulation as an automatic differentiation technique for MLP networks was first introduced in the 1970s and 1980s and later became known as backpropagation.
...
However, in Linnainaa's work, the concept of reverse accumulation was studied in cumulative floating point errors.
...
Then mark the output of layer $\ell$ as $z^{(\ell)} = W^{(\ell)} a^{(\ell-1)} + b$ where $a^{(\ell-1)} =  \phi^{\ell-1}(z^{(\ell-1)})$ is the output from the activation of the previous layer.
...
\end{equation}
Then, the $z$ derivatives are easily evaluated for the weight terms to
\begin{equation}
    \frac{\partial J}{\partial W_{ij}^{(\ell)}} = \frac{\partial J}{\partial z_{j}^{(\ell)}} z_{i}^{(\ell)},
\end{equation}
...
\end{equation}
What is left is to find an equation for the gradient term of~\eqref{eq:mlchainrulevec} that is often called the local gradient or error.
...
Now that the neural network can be constructed, there is a way to find the optimal network parameters by computing easily the gradients, and a complete neural network can be trained.
