In this subsection, the very basics of neural networks are covered starting from the McCulloch-Pitts Neuron and training it to backpropagation to the modern convolutional neural networks.
As the material of this subsection covers also the elementary underlying principles of neural networks, it is also above the bare minimum needed for understanding the results of this work but provides still some context to later discussions.
But as in this work neural networks are forced to learn a partial differential equation in a departure from the mainstream neural network applications, the knowledge of their inner workings is crucial in understanding what can be done and what kind of structes could potentially work. 
...
\begin{figure}
    ...
    \caption{\label{fig:mccullochpittsneuron}McCulloch-Pitts Neuron takes a binary input vector evaluates their sum, and determines the binary output by comparing the sum to a given threshold.}
\end{figure}
...
\begin{align}
    y = \phi(f(\mathbf{x}))  = \mathbf{1}_{\leq T} (f(\mathbf{x})) & = 1 \quad \text{if } f(\mathbf{x}) \geq T \\
...
\begin{figure}
    ...
    \caption{\label{fig:perceptron}Perceptron now allows any input values with a bias $b$ and with a more sophisticated activation function $\phi$ the output value can be anything.}
\end{figure}
...
\begin{figure}
    ...
    \caption{\label{fig:activations}Different commonly used activation functions.}
\end{figure}
...
\begin{figure}
    ...
    \caption{\label{fig:mlp}Multilayer Perceptron is now multiple perceptrons combined in layers.
        This figure has one hidden layer consisting of two perceptrons and thus the output is two-dimensional.
        MLP uses this way more weights to learn more complex patterns from data.}
\end{figure}
...
In Figure~\ref{fig:2dconv}, a convolution of larger matrix with a $3 \times 3$ kernel is visualised.
...
\end{equation}
Depending on the application, the standard loss functions are sometimes augumented with application-specific values.
For instance in the realm of the physics-informed neural networks, it is customary to add to the used loss functions some physical quantities to be minimised. 
...
Typically most of these things presented are encapsulated inside the numerical neural network libraries the most-well known examples being the Python libraries PyTorch and Tensorflow.
