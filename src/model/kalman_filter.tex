...
The filtering can be performed over a larger domain than necessary and focus on the interior of the filtered states so that observations would be assimilated multiple times for the interior pixels.
Also wrong border values are over time corrected with observations by the Kalman filter.
...
Consequently, the ensemble generation has more freedom when generating only predictions.
The ensemble can in the prediction case then be generated and inflated on a case-by-case basis depending on the exact data source used and e.g.\ on the quality of previous predictions.
