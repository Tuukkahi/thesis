...
\end{equation*}
where $I_t \in \R^{n \times n}$ is an image at time $t$, and $\mathbf{F}$ a vector field of the $u$ and $v$ basis components of motion vectors.
...
The decision to dismiss diffusion from this model primarily stems from the fact that the used cloud data does not exhibit diffusion and thus the diffusion process lacks physical interpretation.
...
To create two-dimensional motion vectors, two separate ResNet models are employed, each trained to predict the two basis components of the two-dimensional vector field.
...
\end{equation}
where $f_{\operatorname{res}}(x)$ is the residual mapping to be learned representing neural network layers. $x$ is the block's original input, representing the skip connection.
...
To this end, Lagrange polynomial interpolation is used to extend the predicted vector across the entire domain.
The method of predicting motion vector values from which the polynomial coefficients are then computed gives stability the optimisation process.
This way small changes to the network output has predictable small changes in the complete vector fields and thus in the loss values.
...
\begin{figure}[h]
    ...
    \caption{\label{fig:testpoints}The six nodes at which the neural network approximated velocity field values. In order to ensure stable invertibility of the two-dimensional Vandermonde matrix, the location of the nodes must be selected carefully.}
\end{figure}
